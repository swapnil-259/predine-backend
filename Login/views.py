    ...
    if request_handlers.request_type(request, "POST"):
        ...
        if validate_data is not None:
            if validate_data["status"] == True and validate_data["other"] == False:
                return JsonResponse(
                    {"msg": validate_data["msg"] + " " + status_message.REQUIRED},
        ...
        try:
            ...
            return JsonResponse(
                {"msg": "User not found."}, status=status_code.BAD_REQUEST
            )

        user = authenticate(username=username, password=password)
        if user is not None:
            check_email_verified = (
                OTPDetails.objects.filter(email=username).order_by("-id").first()
            )
            if check_email_verified is not None:
                if not check_email_verified.verified_status:
                    return JsonResponse(
                        {"msg": status_message.EMAIL_NOT_VERIFIED},
                        status=status_code.BAD_REQUEST,
                    )
            else:
                return JsonResponse(
            ...
            return JsonResponse(
                {"msg": status_message.WRONG_CREDENTIALS},
                status=status_code.BAD_REQUEST,
            )
    else:
        return JsonResponse(
...
def user_registration(request):
    if request_handlers.request_type(request, "POST"):
        ...
        if confirm_password != password:
            return JsonResponse(
                {"msg": status_message.PASSWORD_NOT_MATCH},
                status=status_code.BAD_REQUEST,
            )
        if User.objects.filter(username=email, deleted_status=False).exists():
            return JsonResponse(
                {"msg": status_message.USER_ALREADY_REGISTERED},
                status=status_code.BAD_REQUEST,
            )

        if UserRole.objects.filter(
            user_id__email=email, role_id__role_name="USER"
        ).exists():
            return JsonResponse(
                {"msg": status_message.USER_ALREADY_REGISTERED},
                status=status_code.BAD_REQUEST,
        ...
        if (
            OTPDetails.objects.filter(email=email, verified_status=True).exists()
            is False
        ):
            return JsonResponse(
                {"msg": status_message.EMAIL_NOT_VERIFIED},
        ...
        if user:
            role = UserRole.objects.create(
                role=Roles.objects.get(role_name="USER"), user=user
            )
            login(request, user)
            role = list(
                UserRole.objects.filter(user=user).values(
            ...
            return JsonResponse(
                {"msg": status_code.BAD_REQUEST}, status=status_message.BAD_REQUEST
            )
        if role:
    ...
    if request_handlers.request_type(request, "POST"):
        ...
        if OTPDetails.objects.filter(email=email, verified_status=True).exists():
            if User.objects.filter(username=email).first() is None:
                return JsonResponse(
                    {
                        "msg": "Your Email is verified. Kindly do the further registration",
                ...
                return JsonResponse(
                    {
                        "msg": "Email already Registered. Kindly Login.",
            ...
            if otp_generate:
                ...
                if mail:
                    threading.Thread(
                        target=functions.otp_expire, args=(email, otp)
                    ).start()
                    return JsonResponse(
                        {
                            "msg": status_message.OTP_SENT,
                    ...
                    return JsonResponse(
                        {"msg": status_message.BAD_REQUEST},
                        status=status_code.BAD_REQUEST,
                    )
            else:
                return JsonResponse(
...
def verify_otp(request):
    if request_handlers.request_type(request, "POST"):
        ...
        otp = data.get("otp")
        email = data.get("email")
        validate_data = functions.validate(email=email, otp=otp, api_type="OTP")
        if validate_data is not None:
            if validate_data["status"] == True and validate_data["other"] == False:
                return JsonResponse(
                    {"msg": validate_data["msg"] + " " + status_message.REQUIRED},
        ...
        if OTPDetails.objects.filter(
            otp=otp, email=email, deleted_status=False, verified_status=False
        ).first():
            ...
            return JsonResponse(
                {"msg": status_message.OTP_INVALID}, status=status_code.BAD_REQUEST
            )
    else:
        return JsonResponse(
            {"msg": status_message.METHOD_NOT_ALLOWED},
...
def check_email_verification(request):
    if request_handlers.request_type(request, "GET"):
        ...
        if OTPDetails.objects.filter(email=email, verified_status=True).exists():
            return JsonResponse(
                {"msg": status_message.SUCCESS}, status=status_code.SUCCESS
            )
        else:
            return JsonResponse(
                {"msg": status_message.EMAIL_NOT_VERIFIED},
...
def left_panel(request):
    if request_handlers.request_type(request, "GET"):
        panel_data = LeftPanel.objects.filter(
            role=request.session["role"], deleted_status=False, child=None
        ).values("name", "component", "icon", "order", "icon_type", "title")
