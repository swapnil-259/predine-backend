    ...
    if request_handlers.request_type(request, "POST"):
        ...
        if user:
            dd_exist = Dropdown.objects.filter(
                id=role, deleted_status=False, child__parent="ROLE TYPE "
            ).first()
            role_exist = Roles.objects.filter(
                role_name=dd_exist.parent, deleted_status=False
            ).first()
            type_exist = Dropdown.objects.filter(
                id=type, deleted_status=False, child_id__parent="RESTAURANT TYPE"
            ).first()
            if role_exist is None:
                return JsonResponse(
                    {"msg": status_message.INVALID_DATA}, status=status_code.BAD_REQUEST
                )
            ...
            role = UserRole.objects.create(role=role_exist, user=user)
            ...
            return JsonResponse(
                {"msg": status_message.BAD_REQUEST}, status=status_message.BAD_REQUEST
            )
        if owner:
        ...
        try:
            ...
            return JsonResponse(
                {"msg": status_message.USER_NOT_FOUND}, status=status_code.BAD_REQUEST
            )
...
def get_role_type(request):
    if request_handlers.request_type(request, "GET"):
        data = Dropdown.objects.filter(
            child=Dropdown.objects.filter(parent="ROLE TYPE").first()
        ).values("parent", "id")
        transformed_data = [
...
def view_owners(request):
    if request_handlers.request_type(request, "GET"):
        ...
        if not owner_id:
            return JsonResponse(
                {"data": "ID parameter is missing"}, status=status_code.BAD_REQUEST
            )

        restaurant_data = (
            OwnerDetails.objects.filter(deleted_status=False, id=owner_id)
            .values(
                "id",
                "restaurant_name",
        ...
        formatted_data = [
            ...
            {
                "label": "Email",
                "value": restaurant_data["owner__email"],
                "can_edit": False,
                "key": "email",
            },
            {
                "label": "Phone Number",
                "value": str(restaurant_data["owner__phone_number"]),
                "can_edit": True,
                "key": "phone_number",
            },
            {
                "label": "Owner Role",
                "value": str(restaurant_data["owner_role__role_name"]),
                "can_edit": False,
                "key": "role",
            },
            {
                "label": "Restaurant Image",
                "value": str(restaurant_data["restaurant_pic"]),
                "can_edit": True,
                "key": "img",
            },
        ]
