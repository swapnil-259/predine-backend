...
def verification_email(email, otp):
    ...
    html_message = render_to_string("email/send_otp.html", context)
...
def otp_expire(email, otp):
    ...
    OTPDetails.objects.filter(otp=otp, email=email, deleted_status=False).update(
    ...
    match api_type:
        case "OWNER":
            ...
            if not re.match(mobile_pattern, phone_number):
                return JsonResponse(
                    {"msg": status_message.NUMBER_INVALID},
